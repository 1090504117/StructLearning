  ...
  public:
    ...
    void remove( const Comparable & x )
    {
        ...
        else
        {
            ...
            newTree = root->left;
            splay( x, newTree );		//再次展开是因为左侧值一定比x小，然后展开可以将左子节点的右子节点中大的值调高，将根节点更新，之后此节点的右节点一定为空，可以将其指向要删除的节点的右侧，完成树的更新
...
private:
   ...
   void printTree( BinaryNode *t ) const
    ...
    void rotateWithLeftChild( BinaryNode * & k2 )
    {
        ...
        k2->left = k1->right;	//k1->right可能是空树，但是不影响
        k1->right = k2;
        k2 = k1;//因为k2,k1是结构体，这里相当于是更新k2节点为k1节点的值（个人觉得这样的写法不是很好，但是因为要更新节点的指向问题，所以使用k2=k1，不是更新父节点的对k1指向至k2，而是将k2的信息替换原k1位置的信息，和c语言描述有区别）
    }
    ...
    void splay( const Comparable & x, BinaryNode * & t )
    {
        ...
        for( ; ; )
            ...
            {
                if( t->right->element < x )		//当t-right->element == x时，不会进行旋转操作，会将这步
                    rotateWithRightChild( t );
                if( t->right == nullNode )
                    break;
                // Link Left
                leftTreeMax->right = t;	//leftTreeMax->right = t、leftTreeMax = t;用来更新header
                leftTreeMax = t;
                t = t->right;
            }
            else
